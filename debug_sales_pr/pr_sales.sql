...
-- CREATE DISCOUNT ORDER PROCEDURE
SELECT DISTINCT
ord.BranchID,
ord.OrderNbr,
d.InvtID,
...
INNER JOIN dbo.OM_SalesOrdDet d WITH (NOLOCK) ON
d.BranchID = ord.BranchID AND
d.OrderNbr = ord.OrderNbr
INNER JOIN dbo.OM_OrdDisc dis WITH (NOLOCK) ON
dis.BranchID = d.BranchID AND
dis.OrderNbr = d.OrderNbr AND
d.LineRef IN (SELECT part FROM dbo.fr_SplitStringMAX(dis.GroupRefLineRef, ','))
INNER JOIN dbo.OM_DiscSeq sq WITH (NOLOCK) ON
sq.DiscID = dis.DiscID AND
sq.DiscSeq = dis.DiscSeq
INNER JOIN dbo.#Sales s WITH (NOLOCK) ON
ord.BranchID = s.BranchID AND
ord.OrigOrderNbr = s.OrderNbr
...
-- Create #TDiscFreeItem
DROP TABLE IF EXISTS #TDiscFreeItem
...
WHERE FreeItemID <> ''
AND d.FreeItem = 1;
--ENDING INSERT #TDiscFreeItem

--START INSERT NEW ITEMS TO #TDiscFreeItem  
INSERT INTO #TDiscFreeItem
(
    BranchID,
    OrderNbr,
...
sq.Descr
FROM #Sales bat
INNER JOIN dbo.OM_SalesOrd ord WITH (NOLOCK) ON
ord.BranchID = bat.BranchID AND
bat.OrderNbr = ord.OrigOrderNbr
INNER JOIN dbo.OM_SalesOrdDet d WITH (NOLOCK) ON
d.BranchID = ord.BranchID AND
d.OrderNbr = ord.OrderNbr
INNER JOIN #TCpnyID r WITH (NOLOCK) ON
r.CpnyID = ord.BranchID
INNER JOIN dbo.OM_PDAOrdDisc pdis WITH (NOLOCK) ON
pdis.BranchID = d.BranchID AND
pdis.OrderNbr = d.OrigOrderNbr AND
d.InvtID = pdis.FreeItemID AND
d.FreeItem = 1 AND
d.OriginalLineRef = pdis.SOLineRef
INNER JOIN dbo.OM_DiscSeq sq WITH (NOLOCK) ON 
sq.DiscID = pdis.DiscID AND 
sq.DiscSeq = pdis.DiscSeq
LEFT JOIN #TDiscFreeItem dis WITH (NOLOCK) ON 
dis.BranchID = d.BranchID AND 
dis.FreeItemID = d.InvtID AND 
d.OrderNbr = dis.OrderNbr AND 
d.FreeItem = 1 AND 
dis.SOLineRef = d.LineRef
...
--}
LEFT JOIN #TOrdDisc dis WITH (NOLOCK) ON
dis.BranchID = a.BranchID AND
dis.OrderNbr = a.MaCT AND
dis.LineRef = a.LineRef
LEFT JOIN #TDiscFreeItem dis1 WITH (NOLOCK) ON 
dis1.BranchID = a.BranchID AND 
dis1.OrderNbr = a.MaCT AND 
dis1.FreeItemID = a.InvtID AND 
dis1.SOLineRef = a.LineRef
LEFT JOIN dbo.SI_ReasonCode sr WITH (NOLOCK) ON 
sr.ReasonID = a.ReasonCode
--}
LEFT JOIN #Deli d ON 
d.BranchID = a.BranchID AND 
d.OrderNbr = a.OrderNbr
LEFT JOIN #Book iss ON 
iss.BranchID = a.BranchID AND 
iss.OrderNbr = a.OrderNbr
LEFT JOIN #Book iss1 ON 
iss1.BranchID = a.BranchID AND 
iss1.OrderNbr = a.ReturnOrder
INNER JOIN #Customer cu WITH (NOLOCK) ON 
cu.CustId = a.CustID
INNER JOIN dbo.Users sa WITH (NOLOCK) ON 
sa.UserName = a.SlsperID
INNER JOIN dbo.IN_Inventory invt WITH (NOLOCK) ON 
invt.InvtID = a.InvtID
LEFT JOIN dbo.SYS_Company com WITH (NOLOCK) ON 
a.BranchID = com.CpnyID
-- no need {
LEFT JOIN dbo.OM_DebtAllocateDet da WITH (NOLOCK) ON
da.BranchID = a.BranchID AND
da.OrderNbr = a.OrderNbr AND
a.InvcNbr = da.InvcNbr AND
a.InvcNote = da.InvcNote
-- Nguoi chiu trach nhiem no
LEFT JOIN dbo.OM_OriginalContract con WITH (NOLOCK) ON
con.ContractID = a.ContractID
-- So Hop Dong}
LEFT JOIN dbo.Users deli WITH (NOLOCK) ON --nothing
deli.UserName = iss.SlsperID
LEFT JOIN dbo.Users cre WITH (NOLOCK) ON -- nguoi tao don
cre.UserName = a.Crtd_User
LEFT JOIN dbo.Users foll WITH (NOLOCK) ON --nguoi chieu trach nhiem no
foll.UserName = da.SlsperID
LEFT JOIN dbo.Users sup WITH (NOLOCK) ON
sup.UserName=a.SupID
LEFT JOIN dbo.Users asm WITH (NOLOCK) ON
asm.UserName=a.ASM
LEFT JOIN dbo.Users Rsm WITH (NOLOCK) ON
rsm.UserName =a.RSM
--where   (cu.Territory LIKE CASE WHEN @Terr = '' THEN '%' END OR cu.Territory IN (SELECT part FROM dbo.fr_SplitStringMAX(@Terr,',')))
ORDER BY a.BranchID,
a.OrderDate,
