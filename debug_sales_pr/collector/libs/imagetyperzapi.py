...
RECAPTCHA_RETRIEVE_ENDPOINT = (
    "http://captchatypers.com/captchaapi/GetRecaptchaText.ashx"
)
...
CAPTCHA_ENDPOINT_CONTENT_TOKEN = (
    "http://captchatypers.com/Forms/UploadFileAndGetTextNEWToken.ashx"
)
CAPTCHA_ENDPOINT_URL_TOKEN = (
    "http://captchatypers.com/Forms/FileUploadAndGetTextCaptchaURLToken.ashx"
)
RECAPTCHA_SUBMIT_ENDPOINT_TOKEN = (
    "http://captchatypers.com/captchaapi/UploadRecaptchaToken.ashx"
)
RECAPTCHA_RETRIEVE_ENDPOINT_TOKEN = (
    "http://captchatypers.com/captchaapi/GetRecaptchaTextToken.ashx"
)
...
PROXY_CHECK_ENDPOINT_TOKEN = (
    "http://captchatypers.com/captchaAPI/GetReCaptchaTextTokenJSON.ashx"
)
GEETEST_SUBMIT_ENDPOINT_TOKEN = (
    "http://captchatypers.com/captchaapi/UploadGeeTestToken.ashx"
)
...
class Geetest:
    ...
    def response(self):
        ...
        if len(s) == 3:
            return {"challenge": s[0], "validate": s[1], "seccode": s[2]}
        else:
            return self._response
...
class ImageTyperzAPI:
    def __init__(self, access_token, affiliate_id=0, timeout=120):
        ...
        self._headers = {"User-Agent": USER_AGENT}  # use this user agent
    ...
    def solve_captcha(
        ...
        if self._username:
            ...
            if not os.path.exists(image_path):
                raise Exception("captcha image does not exist: {}".format(image_path))
            # read image/captcha
            ...
            if image_path.lower().startswith("http"):
                ...
                if not os.path.exists(image_path):
                    raise Exception(
                        "captcha image does not exist: {}".format(image_path)
                    )
        ...
        response = self._session.post(
            url, data=data, headers=self._headers, timeout=self._timeout
        )
    ...
    def submit_recaptcha(self, d):
        ...
        proxy = None
        if "proxy" in d:
            proxy = d["proxy"]  # if proxy, add it

        # check if page_url and sitekey are != None
        if not page_url:
            raise Exception("provide a valid page_url")
        if not sitekey:
            raise Exception("provide a valid sitekey")
        ...
        self._recaptcha = Recaptcha(
            response_text
        )  # init recaptcha obj with captcha_id (which is in the resp)
    ...
    def retrieve_recaptcha(self, captcha_id=None):
        ...
        if not captcha_id:
            if not self._recaptcha:
                raise Exception(
                    "no recaptcha was submited previously, submit a captcha"
                    " first or give captcha_id as argument"
                )  # raise it
            captcha_id = self._recaptcha.captcha_id

        # create params dict (multipart)
        data = {"action": "GETTEXT", "captchaid": captcha_id}
    ...
    def submit_geetest(self, d):
        # check if page_url and sitekey are != None
        if "domain" not in d:
            raise Exception("domain is missing")
        if "challenge" not in d:
            raise Exception("challenge is missing")
        if "gt" not in d:
            raise Exception("gt is missing")
        ...
        # affiliate ID
        if self._affiliate_id:
            d["affiliateid"] = self._affiliate_id

        url = "{}?{}".format(url, urlencode(d))
        # make request with all data
        response = self._session.post(
            url, data=d, headers=self._headers, timeout=self._timeout
        )
    ...
    def retrieve_geetest(self, captcha_id=None):
        # if captcha id is not specified, use the ID of the last captcha submited
        if not captcha_id:
            if not self._geetest:
                raise Exception(
                    "no geetest was submited previously, submit a captcha"
                    " first or give captcha_id as argument"
                )  # raise it
    ...
    def set_captcha_bad(self, captcha_id=""):
        ...
        if not self._normal_captcha and not captcha_id:
            raise Exception("no captcha id given and no captcha solved before")
        if not captcha_id:
            captcha_id = (
                self._normal_captcha.captcha_id
            )  # if no captcha id given, use last id
        data = {"action": "SETBADIMAGE", "imageid": captcha_id, "submit": "Submissssst"}
    ...
    def was_proxy_used(self, captcha_id):
        ...
        if (
            len(js["Proxy_client"].split(":")) >= 2
            and js["Proxy_client"] == js["Proxy_worker"]
        ):
    ...
    def captcha_text(self):
        if not self._normal_captcha:
            return ""  # if captcha is not set yet, return nothing
    ...
    def recaptcha_response(self):
        if not self._recaptcha:
            return ""  # return nothing if not set yet
