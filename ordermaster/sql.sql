...
/****** Object:  StoredProcedure [dbo].[pr_OM_RawdataPDAOrderALL]    Script Date: 18/05/2022 12:12:31 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROC [dbo].[pr_OM_RawdataPDAOrderALL]   -- pr_OM_RawdataPDAOrderALL_Test  9142
@RPTID INT
AS
...
----DECLARE @RPTID INT = 699
    ...
    SELECT  @fromdate = DateParm00 ,
            @todate = DateParm01 , @UserID = UserID  ,
			@Zone = StringParm00 , @Terr = StringParm01
...
---khoahnt them
CREATE TABLE #TableBranchID (BranchID VARCHAR(50))
  INSERT INTO #TableBranchID
	    (
	        BranchID
	    )
		SELECT BranchID = StringParm FROM dbo.RPTRunningParm0 WITH(NOLOCK) WHERE ReportID = @RPTID

SELECT * 
INTO #Customer
FROM dbo.vs_AR_CustomerInfo cu WITH(NOLOCK)
INNER JOIN dbo.RPTRunningParm0 r WITH(NOLOCK) ON r.StringParm = cu.BranchID AND r.ReportID = @RPTID
WHERE (cu.Territory LIKE CASE WHEN @Terr = '' THEN '%' END OR cu.Territory IN (SELECT part FROM dbo.fr_SplitStringMAX(@Terr,',')))
AND (cu.Zone LIKE CASE WHEN @Zone = '' THEN '%' END OR cu.Zone IN (SELECT part FROM dbo.fr_SplitStringMAX(@Zone,',')))
